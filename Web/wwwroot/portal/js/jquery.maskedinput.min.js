...
$.fn.maskinput = function (mask) {
    ...
    $(this).attr({
        "onkeydown": "return($(this).validate())"
    });
    if ($("form").find("#__maskTest").length == 0) {
        var __check = $("<input>").attr({
            "type": "text",
            "id": "__maskTest",
            "name": "__maskTest"
        }).css({
        ...
        $(__check).appendTo("form");
    }
}
$.fn.validate = function () {
    //console.log($(this).attr("pattern"));
    if ($(this).attr("pattern") != undefined && event.key.length == 1) {
        $("#__maskTest").attr("pattern", $(this).attr("pattern"));
        $("#__maskTest").val($(this).val() + event.key);
        //console.log($("#__maskTest")[0].checkValidity());
        return ($("#__maskTest")[0].checkValidity());
    }
    return true;
}
    ...
    a.mask = {
        ...
        caret: function (a, b) {
            ...
            var h, i, j, k, l, m, n, o; if (!c && this.length > 0) {
                ...
                } var B = a(this), C = a.map(c.split(""), function (a, b) { return "?" != a ? i[a] ? p(b) : a : void 0; }), D = C.join(""), E = B.val(); B.data(a.mask.dataName, function () { return a.map(C, function (a, b) { return j[b] && a != p(b) ? a : null; }).join(""); }), B.one("unmask", function () { B.off(".mask").removeData(a.mask.dataName); }).on("focus.mask", function () {
                    if (!B.prop("readonly")) {
                        clearTimeout(b);
                        var a;
